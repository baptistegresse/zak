...
/*   Updated: 2023/03/10 16:56:38 by bgresse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_read_line(int fd, char *cache)
{
	...
	while (readlen != 0 && !ft_strchr2(cache, '\n'))
	{
		...
		if (readlen == -1)
			return (ft_free_remove(FREE, cache), NULL);
		buffer[readlen] = 0;
...
static char	*ft_after_line(char *cache)
{
	...
	while (cache[i])
		str[j++] = cache[i++];
	str[j] = 0;
	return (ft_free_remove(FREE, cache), str);
}
...
char	*get_next_line(int fd)
{
	...
	if (!cache[fd])
		return (ft_free_remove(FREE, cache[fd]), NULL);
